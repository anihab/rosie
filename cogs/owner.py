...
class Owner(commands.Cog):
    ...
    async def _eval(self, context: Context, *, code: str) -> None:
        try:
            result = eval(code)
            await context.send(f"here's the result: {result}", ephemeral=True)
        except Exception as e:
            await context.send(f"oh no! something went wrong... {e}", ephemeral=True)
    ...
    async def blacklist(self, context: Context, user: discord.User) -> None:
        try:
            ...
            await context.send("i'm sorry, but i don't recognize that person", ephemeral=True)
